...
class FlowGraphBuilder:
    ...
    def create_start(self, name="start", **kwargs):
        return self.create_node(name=name, **kwargs, shape="block", style="rounded,filled", color="#CCFFCC")

    def create_action(self, **kwargs):
        return self.create_node(**kwargs, shape="block", style="filled", color="#CCFFFF")

    def create_setup(self, **kwargs):
        # In my opinion a loop is both an action (spit out elements) and a decision (decide if continue).
        return self.create_node(**kwargs, shape="hexagon", style="filled", color="#EEEEEE")

    def create_decision(self, **kwargs):
        return self.create_node(**kwargs, shape="diamond", style="filled", color="#FFCCCC")
    ...
    def create_terminal(self, **kwargs):
        terminal = Node(**kwargs, shape="block", style="filled,rounded")
